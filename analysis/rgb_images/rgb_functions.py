...
def add_scale_bar(ax, deltaPix, length=1, fontsize=18, scale_text=None):
    """
    ...
    ax.plot([d_x/20.,d_x/20.+length/deltaPix], [y_max-d_y/20.,y_max-d_y/20.], '-', color='w', linewidth=2)
...
def add_arrows(ax, fontsize=18):
    ...
    ax.arrow(x_max-d_x/20., y_max-d_y/20, -.4/deltaPix, 0, head_width=3, head_length=3, fc='white', ec='white', linewidth=1.5)
    ax.arrow(x_max-d_x/20., y_max-d_y/20, 0, .4/deltaPix, head_width=3, head_length=3, fc='white', ec='white', linewidth=1.5)
    
    ax.text(x_max-d_x/20. - 1.38*0.5/deltaPix, y_max-d_y/25, "E", color='white', ha='center', fontsize=fontsize)
    ax.text(x_max-d_x/20., y_max-d_y/20 + 1.3*0.5/deltaPix, "N", color='white', ha='center', fontsize=fontsize)
    
    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_max, y_min)
    

def plot_quasar_grid(quasar_data_list, names=None, weights=[1., .5, .15], rolls=None, rollalls=None, zoom=False, ruler_length=1, fontsize=18, size=(24,15)):
    """
    Plot a grid of RGB images for multiple quasars.

    Parameters:
    - quasar_data_list: list of tuples/lists, each with [R, G, B] image arrays for one quasar
    - names: optional list of names for labeling each image
    - weights: RGB scaling weights
    - rolls: optional list of roll instructions per quasar
    - rollalls: optional list of rollall instructions per quasar
    - zoom: zoom option for each subplot
    """
    fig, axes = plt.subplots(2, 4, figsize=size)
    axes = axes.flatten()

    for i, data in enumerate(quasar_data_list):

        ax = axes[i]
        roll = rolls[i] if rolls else None
        rollall = rollalls[i] if rollalls else None
        name = names[i] if names else f"Quasar {i+1}"

        make_rgb_image(name=name, data=data, ax1=ax,
                       weights=weights[i], roll=roll, rollall=rollall,
                       zoom=zoom, ruler_length=ruler_length, fontsize=fontsize)
        
        name_text(ax, name, fontsize=fontsize)
...
def make_rgb_image(name, data, ax1, weights=[1., 1., 1.], roll=None, sep=15, 
                   rollall=None, zoom=False, ruler_length=20, fontsize=18, scale_text=None):
    """
...
#         pass
        add_scale_bar(ax1, deltaPix=0.04,
                      length=ruler_length, 
                      fontsize=fontsize, scale_text=scale_text)
